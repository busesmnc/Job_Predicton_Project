...
CATEGORIES_PATH = '/Users/busesomunncu/Desktop/Linkedln Job Prediction Project/final_annotated_categories_and_jobs.csv'

# Features list & weights for total percentage
FEATURES = [
...
def main():
    ...
    # Print best matching category for each feature
    print("\n=== Her Özellik İçin En Yüksek Uygun Kategori ===")
    for feat in FEATURES:
        best_id = pct_df[feat].idxmax()
        print(f"- {feat}: {best_id} ID'li kategori")

    # Ask user for most important criterion
    print("\nSenin için en önemli kriter ne?")
    for i, feat in enumerate(FEATURES, start=1):
        print(f"{i} - {feat}")
    choice = int(input("Seçiminizi girin (1-6): ").strip())
    chosen = FEATURES[choice-1]

    # Select top 7 categories by chosen criterion with technical tiebreaker
    sorted_df = pct_df.sort_values(by=[chosen, 'technical_requirements'], ascending=[False, False])
    top7 = sorted_df.head(7)

    # Calculate and add weighted total percentage for these 7
    top7['total_percentage'] = sum(top7[feat] * WEIGHTS[feat] for feat in FEATURES)

    print(f"\n=== Aday 7 Kategori ve Ağırlıklı Toplam (Seçilen Kriter: {chosen}) ===")
    print(top7)

    # Finally, recommend top 5 by total_percentage
    top5 = top7.sort_values(by='total_percentage', ascending=False).head(5)
    print("\n=== Önerilen 5 Kategori (Ağırlıklı Toplam Sırasıyla) ===")
    print(top5[['total_percentage']])

if __name__ == "__main__":
    main()

# Execute main to run the workflow
# main()
